...
/*/{Protheus.doc} ImpPA
description
Importação de Perfil aprovadores
@type function
@version
@author Helton.Silva
@since 08/12/2023
...
Static Function ProcessCSV(cCaminho,oProcess)
	...
	For nX:=2 to len(aLines)
		...
		If !Empty(cLinha) .And. !Empty(StrTran(cLinha,';',''))
			...
			If Len(aLinha) > 0
                Update(aLinha[1],aLinha[2],aLinha[3],aLinha[4],aLinha[5],aLinha[6],nX)
			EndIf
		EndIf
	Next
...
Static Function Update( cFil, cCod, cDescri, nLimMin, nLimMax,nMoeda,nLinha)
    Local oModel    := Nil
    Local aMsgDeErro:= {}

    DbSelectArea('DHL')
    DHL->(DbSetOrder(1))
    If !DHL->(DbSeek(Padr(cFil,TamSX3('DHL_FILIAL')[1])+cCod))//Posicionar na DHL para realizar a inclusão

        oModel := FWLoadModel("COMA210")
        oModel:SetOperation(3)
		
        If(oModel:CanActivate())           
            oModel:Activate()
            oModel:SetValue("ModelDHL","DHL_FILIAL"  , cFil	  )
            oModel:SetValue("ModelDHL","DHL_COD"     , cCod	  )
            oModel:SetValue("ModelDHL","DHL_DESCRI"  , cDescri)
            oModel:SetValue("ModelDHL","DHL_LIMMIN"  , Val(nLimMin))
            oModel:SetValue("ModelDHL","DHL_LIMMAX"  , Val(nLimMax))
            oModel:SetValue("ModelDHL","DHL_MOEDA"   , Val(nMoeda) )

            If (oModel:VldData()) /*Valida modelo como um todo*/
                oModel:CommitData()
            EndIf
        EndIf
        ...
        If(oModel:HasErrorMessage())
			...
			cError += "Nao foi possivel cadastrar o Perfil de aprovador "+cCod+" - Linha "+cValToCHar(nLinha)+CRLF
        EndIf
		
		oModel:DeActivate()                         
    Else
		cError += "Perfil de Aprovador ja cadastrado "+cCod+" - Linha "+cValToCHar(nLinha)+CRLF
    EndIf  
