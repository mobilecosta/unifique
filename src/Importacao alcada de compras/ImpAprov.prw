...
/*/{Protheus.doc} ImpAprCsv
description
Importação de aprovadores
@type function
@version
@author Helton.Silva
@since 08/12/2023
...
Static Function ProcessCSV(cCaminho,oProcess)
	...
	For nX:=2 to len(aLines)
		...
		If !Empty(cLinha) .And. !Empty(StrTran(cLinha,';',''))
			...
			If Len(aLinha) > 0
                Update(aLinha[1],aLinha[2],aLinha[3],aLinha[4],aLinha[5],aLinha[6],aLinha[7],aLinha[8],aLinha[9], nX)
			EndIf
		EndIf
	Next
...
Static Function Update( cFil, cCod, cNome, cUser, nLimite, cAprovSup, nMoeda, cTipo, cLogin,nLinha)
    Local oModel    := Nil
    Local aMsgDeErro:= {}

    DbSelectArea('SAK')
    SAK->(DbSetOrder(1))
    If !SAK->(DbSeek(Padr(cFil,TamSX3('AK_FILIAL')[1])+cCod))//Posicionar na SAK para realizar a inclusão

        oModel := FWLoadModel("MATA095")
        oModel:SetOperation(3)

        If(oModel:CanActivate())           
            oModel:Activate()
		//AK_FILIAL,AKCODIGO,AKUSUARIO,AKNOME,AK_LIMITE,AK_APROVSUP,AKMOEDA,AKTIPO,AKLOGIN
            oModel:SetValue("SAKMASTER","AK_FILIAL"  , cFil)
            oModel:SetValue("SAKMASTER","AK_COD"     , cCod)
            oModel:SetValue("SAKMASTER","AK_USER"    , cUser)
            oModel:SetValue("SAKMASTER","AK_NOME"    , AllTrim(cNome))
            oModel:SetValue("SAKMASTER","AK_LIMITE"  , nLimite)
            oModel:SetValue("SAKMASTER","AK_APROSUP" , cAprovSup)
            oModel:SetValue("SAKMASTER","AK_MOEDA"   , nMoeda)
            oModel:SetValue("SAKMASTER","AK_TIPO" 	 , cTipo)
            oModel:SetValue("SAKMASTER","AK_LOGIN"  , cLogin)         
        ...
        If(oModel:HasErrorMessage())
			...
			cError += "Nao foi possivel cadastrar o aprovador "+cCod+" - Linha "+cValToCHar(nLinha)+CRLF
        EndIf
		
		oModel:DeActivate()                         
    Else
		cError += "Aprovador ja cadastrado "+cCod+" - Linha "+cValToCHar(nLinha)+CRLF
    EndIf  
