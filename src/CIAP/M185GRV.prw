...
User Function M185GRV()
	...
	//ponto de entrada na baixa de pré-requisição

	If SD3->D3_TM  == SUPERGETMV("MV_XTPCIAP", .F., "",)
		cProd   :=  SCP->CP_PRODUTO
		cLot    :=  SCP->CP_LOTE
		nQuant  :=  SCP->CP_QUANt
		...
		While TMP->(!EOF())
			...
			//qtdeD3/QuantD1*icms
			GeraCiapSA(cProduto, cCodFor, cLoja, cDocNF, cSerNF,cTipDoc, dDtDg, dDtEmi, nValIcms, nQuant,cItem)
			TMP->(DBSKIP())
		End
	Endif
Return lRet

Static Function GeraCiapSA(cProduto, cCodFor, cLoja, cDocNF, cSerNF,cTipDoc, dDtDg, dDtEmi, nValIcms, nQtde,cItem)
