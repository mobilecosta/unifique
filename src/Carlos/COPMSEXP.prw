...
Static Function RETDADOS(cPlan)
	...
	Case cPlan == "EDT"
		...
		cQuery += " 	AFC_EDTPAI <> '"+Space(TAMSX3("AFC_EDTPAI")[1])+"' AND "
		cQuery += " 	AFC.D_E_L_E_T_ <> '*' "
		cQuery += " ORDER BY AFC_EDTPAI, AFC_EDT "
	Case cPlan == "TAREFAS"
		cQuery := " SELECT "
		cQuery += " 	AF9_EDTPAI, AF9_TAREFA,	AF9_DESCRI,	AF9_UM,	"
		cQuery += " 	AF9_QUANT, AF9_HDURAC, AF9_BDI, AF9_START, AF9_FINISH, AF9_CCUSTO "
	...
	Case cPlan == "PRODUTOS"
		cQuery := " SELECT "
		cQuery += " 	AFA_TAREFA,	AFA_ITEM, AFA_PRODUT, AFA_QUANT, AFA_CUSTD, AFA_DATPRF "
		...
		cQuery += " 	AFA_RECURS = '"+Space(TAMSX3("AFA_RECURS")[1])+"' AND "
	...
	Case cPlan == "DESPESAS"
		cQuery := " SELECT "
		cQuery += " 	AFB_TAREFA,	AFB_ITEM, AFB_TIPOD, AFB_DESCRI, AFB_VALOR,	AFB_DATPRF "
	...
	dbUseArea(.T., "TOPCONN", TcGenQry(,, cQuery), "TSQL")
	...
	Do While TSQL->(!EOF())
		...
		Case cPlan == "TAREFAS"
			aadd(aRetCpos, {TSQL->AF9_EDTPAI, TSQL->AF9_TAREFA,	TSQL->AF9_DESCRI, TSQL->AF9_UM,;
				TSQL->AF9_QUANT, TSQL->AF9_HDURAC, TSQL->AF9_BDI, DTOC(TSQL->AF9_START), DTOC(TSQL->AF9_FINISH), TSQL->AF9_CCUSTO })
		Case cPlan == "PRODUTOS"
			aadd(aRetCpos, {TSQL->AFA_TAREFA, TSQL->AFA_ITEM, TSQL->AFA_PRODUT, TSQL->AFA_QUANT,;
		...
		Case cPlan == "DESPESAS"
			aadd(aRetCpos, {TSQL->AFB_TAREFA, TSQL->AFB_ITEM, TSQL->AFB_TIPOD, TSQL->AFB_DESCRI,;
				TSQL->AFB_VALOR, DTOC(TSQL->AFB_DATPRF)})
		EndCase
